...
def QSort1():
    """
    my implementation of QSort with merging two lists
    i think it's a bit more elegant
    """
    my_list = [random.randint(x, 300) for x in range(30)]
    def qsort1(in_list):
        if in_list == []: 
            return []
        else:
            pivot = in_list[0]
            lesser = qsort1([x for x in in_list[1:] if x < pivot])
            greater = qsort1([x for x in in_list[1:] if x >= pivot])
            return lesser + [pivot] + greater
    print 'before: ', my_list
    print 'after:', qsort1(my_list)
